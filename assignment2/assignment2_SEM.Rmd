...
We then also added a covariance between X1 and X2 to the model which further improved the CFI (0.994) and also the RMSEA (0.15) decreased. The modified structure is visualized below.
The remaining modification indices and residuals imply that further improvement could be yielded by adding the regression of X2 on Y2. Theoretically, this would again not make sense, because shooting efficiency (Y2) is not dependant on assists (X2).



```{r}
# one of our modified models 
model <- '
# regressions
Y1 ~ X1 + X2 + Y2 
Y2 ~ X1 + X3 
...
In our attempt at improving the model, we incrementally added variables, as described before and yielded another model, as seen below, which achieves the statistically best values, with a p-value of 0.019, CFI of 0.998, RMSEA of 0.118 and GFI of 0.999. However, we do not consider the structure useful, as it makes the model overly complex. 
...
X1 ~~ X2
'
fit3 <- lavaan::sem(model,sample.nobs = 320,sample.cov=cov)
...
\section{Conclusion}
We evaluated the fit of a path model consisting of manifest variables using different criteria and concluded that the model is a good representation of the data. We attempted to improve the model and assessed multiple structural changes. 
We compared the different fit of both proposed models using an anova, which revealed that both modifications fit the data significantly better than the base model given in the task. Theoretically speaking, however, the second improved model included some discrepancies, as shooting efficiency is not dependent on the number of assists. 
