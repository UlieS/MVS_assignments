...
When dealing with PCA there is basically one assumption which has to be met, the variables have to be standardized. 

1. scaling

2. Finding the right amount of PCA

Determine the appropriate number of components that can be used to effectively summarize the information in the data. Explain how you settled on the reported number of components.
...
```{r}
ap <- parallel(subject = nrow(drugdata), var = 5, rep= 1000,cent = 0.05)
...
```{r}
pr.out <- prcomp(drugdata, scale = TRUE)
```
